...
Usage:
  ...
  python3 test_magnum_tracking_base_verMultiShape.py  --save-dir /tmp/hybrid_control/ --duration 60 --desired-path rectangle --record-video --object right_triangle
"""
...
PID_DECIMATION = 20  # Update ObjectVelocityPIDController every 5 Phase 7 control cycles
...
APPROACH_DISTANCE = 0.4  # Distance from contact point to spawn robot (for faster testing)
...
def main():
    ...
    parser.add_argument("--object", type=str, default="right_triangle",
                        choices=["right_triangle", "bolt", "pi", "root", "rect"],
    ...
    for _ in range(n_steps):
        ...
        if step_count % CTRL_STEP == 0:
            ...
            if waypoint_controller is not None and object_velocity_pid is not None:
                # Only run high-level waypoint + PID every PID_DECIMATION control cycles
                pid_cycle_count += 1
