...
from contact_maintain.omniwheel_more_real_robot import OmniwheelMoreRealRobot
...
class RobotComparisonTest:
    """Test comparing dummy vs omniwheel robot vs more real robot."""
    ...
    def __init__(self):
        ...
        self.more_real_robot = OmniwheelMoreRealRobot(
            str(Path(pkg_path) / "urdf" / "omniwheel_robot.urdf"),
            position=(1.5, 0),
            orientation=0.0,
            wheel_friction_x=1.0,  # Friction in x-direction (traction)
            wheel_friction_y=0.0,  # Friction in y-direction (free sliding like real omniwheels)
            wheel_motor_force=100.0,
            kp_vel=2.0,
            ki_vel=0.0001,
            dt_ctrl=1.0 / CTRL_FREQ,
        )
        ...
        self.history = {
            ...
            'more_real_vel': [],
            'more_real_wheel_vel': [],
            'more_real_cmd_wheel_vel': [],
        }
    
    def reset(self):
        """Reset all robots."""
        self.dummy_robot.reset(position=(-1.5, 0), orientation=0.0)
        self.omni_robot.reset(position=(0.0, 0), orientation=0.0)
        self.more_real_robot.reset(position=(1.5, 0), orientation=0.0)
    ...
    def run_test(self, trajectory_name='circle', duration=10.0, gui=True):
        ...
        print("  Dummy robot (blue) at x=-1.5")
        print("  Omniwheel robot (orange) at x=0.0")
        print("  More Real robot (green) at x=+1.5")
        ...
        for step in range(n_steps):
            ...
            if step_count % CTRL_STEP == 0:
                ...
                self.more_real_robot.command_velocity(cmd_vel)
                ...
                more_real_pos, more_real_heading, more_real_vel = self.more_real_robot.get_state()
                more_real_wheel_vel = self.more_real_robot.get_wheel_velocities()
                ...
                self.history['more_real_pos'].append(more_real_pos.copy())
                self.history['more_real_heading'].append(more_real_heading)
                self.history['more_real_vel'].append(more_real_vel.copy())
                self.history['more_real_wheel_vel'].append(more_real_wheel_vel.copy())
                self.history['more_real_cmd_wheel_vel'].append(
                    self.more_real_robot.last_wheel_speeds.copy())
    ...
    def calculate_errors(self):
        ...
        more_real_pos = np.array(self.history['more_real_pos'])
        more_real_vel = np.array(self.history['more_real_vel'])
        
        # Velocity tracking error
        dummy_vel_error = np.linalg.norm(dummy_vel - cmd_vel, axis=1)
        omni_vel_error = np.linalg.norm(omni_vel - cmd_vel, axis=1)
        more_real_vel_error = np.linalg.norm(more_real_vel - cmd_vel, axis=1)
        
        # Trajectory deviation (relative to starting offset)
        dummy_traj = dummy_pos - dummy_pos[0]
        omni_traj = omni_pos - omni_pos[0]
        more_real_traj = more_real_pos - more_real_pos[0]
        traj_diff_omni = np.linalg.norm(dummy_traj - omni_traj, axis=1)
        traj_diff_more_real = np.linalg.norm(dummy_traj - more_real_traj, axis=1)
        traj_diff_omni_vs_more_real = np.linalg.norm(omni_traj - more_real_traj, axis=1)
        
        errors = {
            'dummy_vel_rmse': np.sqrt(np.mean(dummy_vel_error**2)),
            'omni_vel_rmse': np.sqrt(np.mean(omni_vel_error**2)),
            'more_real_vel_rmse': np.sqrt(np.mean(more_real_vel_error**2)),
            'trajectory_diff_omni_rmse': np.sqrt(np.mean(traj_diff_omni**2)),
            'trajectory_diff_omni_max': np.max(traj_diff_omni),
            'trajectory_diff_more_real_rmse': np.sqrt(np.mean(traj_diff_more_real**2)),
            'trajectory_diff_more_real_max': np.max(traj_diff_more_real),
            'trajectory_diff_omni_vs_more_real_rmse': np.sqrt(np.mean(traj_diff_omni_vs_more_real**2)),
            'trajectory_diff_omni_vs_more_real_max': np.max(traj_diff_omni_vs_more_real),
        }
    ...
    def plot_results(self, save_path=None, show=False):
        ...
        more_real_pos = np.array(self.history['more_real_pos'])
        more_real_heading = np.array(self.history['more_real_heading'])
        more_real_vel = np.array(self.history['more_real_vel'])
        more_real_wheel_vel = np.array(self.history['more_real_wheel_vel'])
        more_real_cmd_wheel_vel = np.array(self.history['more_real_cmd_wheel_vel'])
        
        fig, axes = plt.subplots(3, 3, figsize=(15, 12))
        fig.suptitle('Robot Comparison: Dummy (blue) vs Omniwheel (orange) vs More Real (green)', fontsize=14)
        ...
        ax.plot(more_real_traj[:, 0], more_real_traj[:, 1], 'g:', linewidth=2, label='More Real')
        ...
        ax.plot(times, more_real_vel[:, 0], 'g:', linewidth=1.5, alpha=0.8, label='More Real')
        ...
        ax.plot(times, more_real_vel[:, 1], 'g:', linewidth=1.5, alpha=0.8, label='More Real')
        ax.set_xlabel('Time (s)')
        ...
        ax.plot(times, more_real_vel[:, 2], 'g:', linewidth=1.5, alpha=0.8, label='More Real')
        ax.set_xlabel('Time (s)')
        ...
        ax.plot(times, np.degrees(more_real_heading), 'g:', linewidth=2, label='More Real')
        ax.set_xlabel('Time (s)')
        ...
        traj_diff_more_real = np.linalg.norm(dummy_traj - more_real_traj, axis=1)
        traj_diff_omni_vs_more_real = np.linalg.norm(omni_traj - more_real_traj, axis=1)
        ax.plot(times, traj_diff_omni * 100, 'r--', linewidth=2, label='Dummy vs Omni')
        ax.plot(times, traj_diff_more_real * 100, 'g:', linewidth=2, label='Dummy vs More Real')
        ax.plot(times, traj_diff_omni_vs_more_real * 100, 'm-', linewidth=2, label='Omni vs More Real')
        ax.set_xlabel('Time (s)')
        ax.set_ylabel('Difference (cm)')
        ax.set_title('Trajectory Differences')
        ax.legend()
        ax.grid(True, alpha=0.3)
        
        # Wheel velocities - Omniwheel
        ax = axes[2, 0]
        labels = ['FR', 'FL', 'RL', 'RR']
        colors = ['r', 'g', 'b', 'm']
        for i in range(4):
            ax.plot(times, omni_wheel_vel[:, i], colors[i] + '-', 
                   linewidth=1.5, label=f'Omni {labels[i]} actual')
            ax.plot(times, omni_cmd_wheel_vel[:, i], colors[i] + '--', 
                   linewidth=1, alpha=0.5, label=f'Omni {labels[i]} cmd')
        ax.set_xlabel('Time (s)')
        ...
        # Wheel velocities - More Real
        ax = axes[2, 1]
        for i in range(4):
            ax.plot(times, more_real_wheel_vel[:, i], colors[i] + '-', 
                   linewidth=1.5, label=f'MoreReal {labels[i]} actual')
            ax.plot(times, more_real_cmd_wheel_vel[:, i], colors[i] + '--', 
                   linewidth=1, alpha=0.5, label=f'MoreReal {labels[i]} cmd')
        ax.set_xlabel('Time (s)')
        ax.set_ylabel('Wheel Velocity (rad/s)')
        ax.set_title('More Real Robot Wheel Velocities')
        ax.legend(fontsize=6, ncol=2)
        ax.grid(True, alpha=0.3)
        ...
        more_real_error = np.linalg.norm(more_real_vel - cmd_vel, axis=1)
        ax.plot(times, dummy_error * 100, 'b-', linewidth=2, label='Dummy')
        ax.plot(times, omni_error * 100, 'r-', linewidth=2, label='Omniwheel')
        ax.plot(times, more_real_error * 100, 'g-', linewidth=2, label='More Real')
        ax.set_xlabel('Time (s)')
...
def main():
    parser = argparse.ArgumentParser(
        description="Compare Dummy vs Omniwheel vs More Real Robot",
...
"""
    ...
    print(f"  More Real velocity RMSE:       {errors['more_real_vel_rmse']*100:.3f} cm/s")
    print(f"  Trajectory diff (Dummy-Omni) RMSE: {errors['trajectory_diff_omni_rmse']*100:.3f} cm")
    print(f"  Trajectory diff (Dummy-Omni) max: {errors['trajectory_diff_omni_max']*100:.3f} cm")
    print(f"  Trajectory diff (Dummy-MoreReal) RMSE: {errors['trajectory_diff_more_real_rmse']*100:.3f} cm")
    print(f"  Trajectory diff (Dummy-MoreReal) max: {errors['trajectory_diff_more_real_max']*100:.3f} cm")
    print(f"  Trajectory diff (Omni-MoreReal) RMSE: {errors['trajectory_diff_omni_vs_more_real_rmse']*100:.3f} cm")
    print(f"  Trajectory diff (Omni-MoreReal) max: {errors['trajectory_diff_omni_vs_more_real_max']*100:.3f} cm")
    print("="*60)
