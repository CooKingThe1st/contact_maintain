#!/usr/bin/env python3
"""
Diff-drive Magnum Four test: zigzag or sine XY via HybridPath + PathFollowingController,
...
Usage:
  python3 test_magnum_diffdrive_control.py --xy-path zigzag --planner hybrid --theta-mode waypoint --save-dir /tmp/dd/
  python3 test_magnum_diffdrive_control.py --xy-path sine --theta-mode segment_tangent --duration 40
  python3 test_magnum_diffdrive_control.py --xy-path zigzag --planner hybrid --theta-mode fixed --save-dir /tmp/holo/ --duration 50 --object root
"""
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="waypoint",
        choices=["waypoint", "fixed", "segment_tangent"],
        help=(
            "waypoint: headings at vertices (e.g. zigzag corners); fixed: constant --fixed-theta; "
            "segment_tangent: vertex heading from HybridPath tangent."
        ),
    )
    parser.add_argument(
        "--fixed-theta",
    ...
    parser.add_argument(
        ...
        default=12.0,
        help=(
            "Max extra wait (s) after retouch duration before advancing (prefer all in_contact). "
            "Increase if corners need longer re-establish contact."
        ),
    )
    parser.add_argument(
        "--hybrid-retouch-allow-incomplete",
        action="store_true",
        help=(
            "Allow leaving retouch on hard timeout even when not all robots report in_contact. "
            "Default False: keeps waiting past soft_end until all contacts or hard timeout."
        ),
    )
    parser.add_argument(
        "--obj-rotate-skip-tol",
        ...
        default=0.06,
        help=(
            "If |wrap(mid_theta - object heading)| is below this (rad), skip ROBOT_ROTATE_A, "
            "OBJECT_ROTATE, and RETOUCH_B (fixed-heading zigzag case)."
        ),
    )
    parser.add_argument(
        "--push-heading-lock",
        action="store_true",
        help=(
            "During PUSH, keep each robot heading near the value set in ROBOT_ROTATE_B "
            "instead of aggressive point-to-contact heading recovery."
        ),
    )
    parser.add_argument(
        "--push-heading-lock-kp",
        type=float,
        default=1.2,
        help="P gain for heading-lock correction during PUSH.",
    )
    parser.add_argument(
        "--push-heading-lock-omega-max",
        type=float,
        default=0.35,
        help="Max |omega| for heading-lock correction during PUSH.",
    )

    parser.set_defaults(push_heading_lock=True)
    parser.add_argument(
        "--no-push-heading-lock",
        dest="push_heading_lock",
        action="store_false",
    ...
    dd_segment_plans: List = []

    dd_theta_mode = {
        "waypoint": DdThetaMode.WAYPOINT,
        "fixed": DdThetaMode.FIXED,
    ...
    for name in robot_agents:
        tp = float(target_map[name])
        inf = contact_point_parameterization.get_contact_info(tp)
        cp_b = np.asarray(inf["point"], dtype=float).reshape(2) - centroid_xy
        n_b = np.asarray(inf["normal_outward"], dtype=float).reshape(2)
        robot_contact_body[name] = (cp_b, n_b)
    ...
    # Delay motion until all robots are pushing; then diff-drive multi-phase segments.
                ...
                if has_ref:
                    ...
                    if holonomic_motion_started:
                        ...
                        if dd_phase == DiffDriveSegmentPhase.RETOUCH_A or dd_phase == DiffDriveSegmentPhase.RETOUCH_B:
                            ...
                            retouch_ok = False
                            if t >= soft_end and all_in_c:
                                retouch_ok = True
                            elif t >= hard_end and args.hybrid_retouch_allow_incomplete:
                                retouch_ok = True
                                if not all_in_c:
                                    print(
                                        f"[dd] WARNING: retouch advance at hard_end WITHOUT all contacts "
                                        f"(allowed by --hybrid-retouch-allow-incomplete) t={t:.2f}s"
                                    )
                            elif t >= hard_end and all_in_c:
                                retouch_ok = True
                            elif t >= soft_end + 50.0:
                                # Escape hatch: avoid deadlock if contact flags never all true
                                retouch_ok = True
                                if not all_in_c:
                                    print(
                                        f"[dd] WARNING: retouch forced after t>soft_end+50s "
                                        f"(in_contact_all={all_in_c}) t={t:.2f}s"
                                    )

                            if retouch_ok:
                                hybrid_retouch_resume_guard = True
                                th_now = float(obj_state["orientation"])
                                mid = float(plan_rot.mid_theta_world)
                                obj_yaw_err = abs(
                                    np.arctan2(
                                        np.sin(mid - th_now),
                                        np.cos(mid - th_now),
                                ...
                                if dd_phase == DiffDriveSegmentPhase.RETOUCH_A:
                                    for rname in robot_agents:
                                        host.robot_states[rname] = RobotState.PUSHING
                                        robot_agents[rname].set_goal("push", target_map[rname])
                                    print(
                                        f"[dd] RETOUCH_A done at t={t:.2f}s in_contact_all={all_in_c} | "
                                        f"seg={dd_next_segment_idx} mid_theta={mid:.4f} "
                                        f"theta_obj={th_now:.4f} |err|={obj_yaw_err:.4f} rad"
                                    )
                                    if obj_yaw_err < OBJ_ROTATE_SKIP_TOL:
                                        dd_phase = DiffDriveSegmentPhase.ROBOT_ROTATE_B
                                        dd_phase_t0 = t
                                        print(
                                            f"[dd]   -> skip object-rotate chain (|mid_theta-theta|<{OBJ_ROTATE_SKIP_TOL}) "
                                            "-> ROBOT_ROTATE_B"
                                        )
                                    else:
                                        dd_phase = DiffDriveSegmentPhase.ROBOT_ROTATE_A
                                        dd_phase_t0 = t
                                        print(
                                            f"[dd]   -> ROBOT_ROTATE_A (need delta_theta~{obj_yaw_err:.4f} rad)"
                                        )
                                else:
                                    dd_phase = DiffDriveSegmentPhase.ROBOT_ROTATE_B
                                    dd_phase_t0 = t
                                    for rname in robot_agents:
                                        ...
                                        robot_agents[rname].set_goal("push", target_map[rname])
                                    print(
                                        f"[dd] RETOUCH_B done -> ROBOT_ROTATE_B at t={t:.2f}s "
                            ...
                            for name, agent in robot_agents.items():
                                ...
                                zeta = robot_heading_goal_co_rotate(th_obj, n_b)
                                _pos, rh, _ = agent.robot.get_state()
                                dd_robot_cmd_override_persistent[name] = robot_rotate_command_diffdrive(
                                    float(rh), zeta
                                )
                            errs = []
                            for name, agent in robot_agents.items():
                                ...
                                zeta = robot_heading_goal_co_rotate(th_obj, n_b)
                                _pos, rh, _ = agent.robot.get_state()
                                errs.append(abs(np.arctan2(np.sin(zeta - rh), np.cos(zeta - rh))))
                            if (errs and max(errs) < ROBOT_HEADING_TOL) or (
                                t - dd_phase_t0 > ROBOT_ROTATE_TIMEOUT_S
                            ):
                                dd_phase = DiffDriveSegmentPhase.OBJECT_ROTATE
                                dd_phase_t0 = t
                                print(
                                    f"[dd] ROBOT_ROTATE_A done (max_err={max(errs) if errs else 0:.3f}) "
                                    f"-> OBJECT_ROTATE mid_theta={plan_rot.mid_theta_world:.3f}"
                                )
                            ...
                            err_th = abs(
                                np.arctan2(
                                    ...
                                    np.cos(plan_rot.mid_theta_world - obj_state["orientation"]),
                                )
                            )
                            if err_th < OBJ_HEADING_TOL or (
                                t - dd_phase_t0 > ROBOT_ROTATE_TIMEOUT_S
                            ):
                                exit_reason = (
                                    "within_tol"
                                    if err_th < OBJ_HEADING_TOL
                                    else "TIMEOUT(phase7/contacts may block in-place yaw)"
                                )
                                ...
                                print(
                                    ...
                                    f"theta_obj={float(obj_state['orientation']):.4f} "
                                    f"err={err_th:.4f} -> RETOUCH_B"
                                )
                            ...
                            for name, agent in robot_agents.items():
                                cp_b, n_b = robot_contact_body[name]
                                zeta = zeta_push_for_robot(plan_b, cp_b, n_b, ROBOT_RADIUS)
                                _pos, rh, _ = agent.robot.get_state()
                                dd_robot_cmd_override_persistent[name] = robot_rotate_command_diffdrive(
                                    float(rh), zeta
                                )
                            ...
                            if (errs and max(errs) < ROBOT_HEADING_TOL) or (
                                ...
                                for name, agent in robot_agents.items():
                                    cp_b, n_b = robot_contact_body[name]
                                    dd_push_heading_lock[name] = float(
                                        zeta_push_for_robot(plan_b, cp_b, n_b, ROBOT_RADIUS)
                                    )
                                print(
                                    f"[dd] ROBOT_ROTATE_B done -> PUSH seg={dd_next_segment_idx} "
                                    f"(max_err={max(errs) if errs else 0:.3f})"
                                )
